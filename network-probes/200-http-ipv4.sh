...
while [ $cnt -le 3 ]; do
	let cnt++
	echo -n check http v4: $1 >> $out_file		#不换行输出“check http v4: 输出的第一个参数”
	x=$(curl -m $TIMEOUT -A "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.83 Safari/537.36" -4 -i $1 2>/dev/null | awk '/^HTTP/ {print $1}')
